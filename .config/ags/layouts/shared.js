...
import { Notch } from './widgets/notch.js';
...
export const Bar = ({ start, centerleft, centerRight, end, anchor, monitor }) => Window({
    name: `bar${monitor}`,
    exclusive: true,
    monitor,
    anchor,
    child: CenterBox({
        className: 'panel',
        startWidget: Box({ children: [...start, ...centerleft], className: 'start' }),
        centerWidget: Box({ child: Notch(), className: 'center' }),
        endWidget: Box({ children: [...centerRight, ...end], className: 'end' }),
    }),
});
