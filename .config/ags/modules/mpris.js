...
export const TitleLabel = ({ player = prefer, ...props } = {}) => 
Label({
    ...props,
    className: 'title',
    connections: [[Mpris, label => {
      let trackTitle = Mpris.getPlayer(player)?.trackTitle
      trackTitle = trackTitle?.replace(/ \([\s\S]*?\)/g, '');
      trackTitle = trackTitle?.length > 40 ? trackTitle?.slice(0,40) + "..." : trackTitle || '';
      trackTitle = trackTitle?.charAt(trackTitle.length - 1) !== " " ? trackTitle + "  " : trackTitle;
      label.label = trackTitle;
    }]],
});
